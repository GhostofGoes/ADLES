...
class VsphereInterface(Interface):
    ...
    def __init__(self, infra, spec):
        ...
        if "thresholds" in infra:
            ...
            self.thresholds = {
                "folder": {"warn": 25, "error": 50},
                "service": {"warn": 50, "error": 70},
            }
        ...
        if "login-file" in infra:
            ...
            self._log.warning(
                "No login-file specified, " "defaulting to user prompts..."
            )
            logins = {}

        # Instantiate the vSphere vCenter server instance class
        self.server = Vsphere(
            username=logins.get("user"),
        ...
        if "server-root" in infra:
            ...
            if not self.server_root:
                self._log.error(
                    "Could not find server-root folder '%s'", infra["server-root"]
                )
    ...
    def _init_groups(self):
        ...
        for name, config in self.spec["groups"].items():
            if "instances" in config:  # Template groups
                groups[name] = [
                    Group(name, config, i) for i in range(1, config["instances"] + 1)
                ]
        ...
        if hasattr(self.server.user_dir, "domainList"):
            self._log.debug(
                "Domains on server: %s", str(self.server.user_dir.domainList)
            )
        return groups
    ...
    def create_masters(self):
        ...
        if not self.template_folder:
            self._log.error(
                "Could not find template folder in path '%s'",
                self.infra["template-folder"],
            )
            return
        else:
            ...
            if sub_name in skip_keys:
                ...
                if "services" in sub_value:  # It's a base folder
                    if self._is_enabled(sub_value):
                        self._log.info(
                            "Generating Master base-type folder %s", sub_name
                        )
                        self._master_base_folder_gen(sub_name, sub_value, new_folder)
                    else:
                        self._log.warning(
                            "Skipping disabled " "base-type folder %s", sub_name
                        )
                else:  # It's a parent folder, recurse
                    if self._is_enabled(sub_value):
                        self._master_parent_folder_gen(sub_value, parent=new_folder)
                        self._log.info(
                            "Generating Master " "parent-type folder %s", sub_name
                        )
                    else:
                        self._log.warning(
                            "Skipping disabled " "parent-type folder %s", sub_name
                        )
    ...
    def _master_base_folder_gen(self, folder_name, folder_dict, parent):
        ...
        for sname, sconfig in folder_dict["services"].items():
            ...
            self._log.info(
                "Creating Master instance '%s' from service '%s'",
                sname,
                sconfig["service"],
            )
            ...
            if vm is None:
                self._log.error(
                    "Failed to create Master instance '%s' " "in folder '%s'",
                    sname,
                    folder_name,
                )
    ...
    def _create_service(self, folder, service_name, networks):
        ...
        if test is None:
            ...
            if vm.is_template():  # Check if it's been converted already
                self._log.warning(
                    "Service %s is a Template, " "skipping configuration", service_name
                )
                return vm
    ...
    def _create_master_networks(self, net_type, default_create):
        ...
        for name, config in self.networks[net_type].items():
            ...
            if exists:
                self._log.info(
                    "PortGroup '%s' already exists on host '%s'", name, self.host.name
                )
            else:  # NOTE: if monitoring, we want promiscuous=True
                self._log.warning(
                    "PortGroup '%s' does not exist on host '%s'", name, self.host.name
                )
                if default_create:
                    self._log.info(
                        "Creating portgroup '%s' on host '%s'", name, self.host.name
                    )
                    create_portgroup(
                        name=name,
                        host=self.host,
    ...
    def deploy_environment(self):
        ...
        self._log.debug(
            "Master folder name: %s\tPrefix: %s",
            self.master_folder.name,
            self.master_prefix,
        )
        ...
        self._deploy_parent_folder_gen(
            spec=self.folders, parent=self.root_folder, path=""
        )
    ...
    def _convert_and_verify(self, folder):
        ...
        for item in folder.childEntity:
            if is_vm(item):
                ...
                self._log.debug(
                    "Unknown item found while " "templatizing Masters: %s", str(item)
                )
        ...
        for sub_name, sub_value in spec.items():
            if sub_name in skip_keys:
                ...
                for i in range(num_instances):
                    ...
                    instance_name = (
                        sub_name if prefix == "" or num_instances == 1 else prefix
                    )
                    ...
                    new_folder = self.server.create_folder(
                        instance_name, create_in=parent
                    )
    ...
    def _deploy_base_folder_gen(self, folder_name, folder_items, parent, path):
        ...
        for i in range(num_instances):
            ...
            # use the folder's name
            instance_name = (
                folder_name if prefix == "" or num_instances == 1 else prefix
            )
            ...
            self._log.info(
                "Generating services for " "base-type folder instance '%s'",
                instance_name,
            )
            self._deploy_gen_services(
                services=folder_items["services"],
                parent=new_folder,
                path=path,
                instance=i,
            )
    ...
    def _deploy_gen_services(self, services, parent, path, instance):
        ...
        for service_name, value in services.items():
            if not self._is_vsphere(value["service"]):
                ...
                continue
            self._log.info(
                "Generating service '%s' in folder '%s'", service_name, parent.name
            )
            ...
            if master is None:  # Check if the lookup was successful
                self._log.error(
                    "Couldn't find Master for service '%s' " "in this path:\n%s",
                    value["service"],
                    path,
                )
                continue  # Skip to the next service
            ...
            for i in range(num_instances):
                instance_name = (
                    prefix + service_name + (" " + pad(i) if num_instances > 1 else "")
                )
                vm = VM(
                    name=instance_name,
                    folder=parent,
    ...
    def _is_vsphere(self, service_name):
        ...
        if service_name not in self.services:
            self._log.error(
                "Could not find service %s in list of services", service_name
            )
        ...
        if net_type == "unique-networks":
            ...
            if net_name not in self.net_table:
                ...
                if exists is not None:
                    self._log.debug(
                        "PortGroup '%s' already exists " "on host '%s'",
                        net_name,
                        self.host.name,
                    )
                    ...
                    self._log.debug(
                        "Creating portgroup '%s' on host '%s'", net_name, self.host.name
                    )
    ...
    def cleanup_masters(self, network_cleanup=False):
        ...
        self._log.info(
            "Found master folder '%s' under folder '%s', " "proceeding with cleanup...",
            master_folder.name,
        ...
        master_folder.cleanup(
            vm_prefix=self.master_prefix,
            recursive=True,
